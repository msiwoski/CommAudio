...
System::Void MyForm::openButton_Click(System::Object^  sender, System::EventArgs^  e) {
	openFileDialog1->Filter = "Streamable files (wav/aif/mp3/mp2/mp1/ogg)\0*.wav;*.aif;*.mp3;*.mp2;*.mp1;*.ogg\0All files\0*.*\0\0";
	...
	if(openFileDialog1->ShowDialog() == System::Windows::Forms::DialogResult::OK ){
		...
		if (str=BASS_StreamCreateFile(FALSE,fileInfo->pfile,0,0,0)) {
			strc++;
			strs=(HSTREAM*)realloc((void*)strs,strc*sizeof(*strs));
			strs[strc-1]=str;
			//STLM(LB_ADDSTRING,0,strrchr(fileInfo->pfile,'\\')+1);
		} else{
